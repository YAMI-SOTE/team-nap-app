...
新規登録アカウントは Google 未連携・予定なしから始まり、設定 > カレンダー連携で
「今すぐ同期」するとサンプル 1 週間分が自分の `CalendarEvent` に取り込まれます。
...
`NapRecord` / `CalendarEvent` は Postgres に永続化されます。
`notifications.service` の通知フィードは引き続きインメモリ（userId ごとの Map）で、
サーバ再起動でリセットされます。
