...
> ### ⚠️ 1 アカウント = 1 端末（先にログインした人が勝ちます）
>
> **すでに誰かがログインしているアカウントには、2 人目はログインできません。**
> 後からログインしようとした側が
> 「すでに別の端末でログインしています。その端末でサインアウトしてから、
> もう一度お試しください」で断られます（409）。先にログインしている端末は
> そのまま使い続けられます。
>
> **審査員が複数人で同時に触る場合は、必ず別々のアカウントを使ってください。**
> そのために 15 名分を用意しています。
>
> **使い終わったらサインアウトしてください。** サインアウトせずにブラウザを
> 閉じるとセッションが残り、次にそのアカウントを使う人が入れません。もし
> 入れなくなったら:
>
> - 元の端末が手元にあれば、そこでサインアウト
> - 手元に無ければ **パスワード再設定**（全セッションが無効になります）
> - あるいは 15 名の中から別のアカウントを使う（いちばん早い）
