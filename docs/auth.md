...
**`createSession` は、そのユーザーの既存セッションをすべて revoke してから新しい
セッションを作る。** つまりアカウントは「最後にログインした端末」だけがサインイン
状態になる。

- 実装位置は各呼び出し側ではなく `createSession` の中。メールログイン /
  サインアップ / Google ログインが自動的に揃い、今後追加される発行経路も
  勝手にこの規則に従う。
- revoke と create は**同一トランザクション**。「古いセッションだけ消えて新しいのが
  発行されない」状態や、同時ログインで両方生き残る状態を作らない。
- 追い出された側の扱い:
  - `lookupSession` が `unknown` / `expired` / `revoked` を区別し、`revoked` のときは
    401 のメッセージが「**別の端末でログインされたため、サインアウトされました**」に
    なる（汎用の「期限切れ」だとユーザーが自分の操作ミスだと誤解するため）。
  - realtime ソケットも即座に切断（`closeUserSockets(userId, "signed in on
    another device")`）。401 に気づく前にチーム更新の受信が止まる。
- 副作用として `logout-others` と `GET /sessions` は実質無意味になった
  （常に 0 件 / 1 件）。モバイル側は元々どちらも使っていない。
