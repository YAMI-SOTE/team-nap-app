...
- Gemma（`OLLAMA_MODEL` で env ごとに指定。既定 `gemma3n:e2b` は ~8GB/2CPU 必要で、
  ローカルでは生成に約 24 秒。小さいホストでは `gemma3:1b`（日本語やや不自然）か、
  未指定でルールベースのみ運用。詳細は [docs/ai-development.md](docs/ai-development.md)）
